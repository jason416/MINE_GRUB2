...
static void print_band(int Xres, int Yres, void *fb);
void grub_test (unsigned long magic, unsigned long addr);
#ifdef COMMENT
...
void grub_test (unsigned long magic, unsigned long addr)
{
    ...
    if (CHECK_FLAG (mbi->flags, 2))
        printf ("cmdline = %s\n", (char *) (unsigned long)mbi->cmdline);
    ...
    if (CHECK_FLAG (mbi->flags, 3))
    {
        ...
        for (i = 0, mod = (multiboot_module_t *)(unsigned long) mbi->mods_addr;
                i < mbi->mods_count;    i++, mod++)
            printf (" mod_start = 0x%x, mod_end = 0x%x, cmdline = %s\n",
                (unsigned) mod->mod_start, (unsigned) mod->mod_end,
                (char *) (unsigned long)mod->cmdline);
    }
    ...
    if (CHECK_FLAG (mbi->flags, 6))
    {
        ...
        for (mmap = (multiboot_memory_map_t *) (unsigned long)mbi->mmap_addr;
       ...
       // unsigned i;
        ...
        switch (mbi->framebuffer_type)
        {
            ...
            case MULTIBOOT_FRAMEBUFFER_TYPE_RGB:
                color = ((1 << mbi->framebuffer_red_mask_size) - 1)
        ...
        #endif
        print_band(mbi->framebuffer_width, mbi->framebuffer_height, fb);
        #if 0
...
{
    int time = Yres / 4;
	int *addr = (int *)fb;  // 32bits 
	int i;

    time++;
	/* each pixel's depth is 32bits */
	for(i = 0 ;i < Xres*20; i++)
	{
		*((char *)addr+0)=(char)0x00;    // 0x00ff0000, RED
	...
	for(i = 0 ;i < Xres*20; i++)
	{
		*((char *)addr+0)=(char)0x00;    // 0x0000ff00, GREEN
	...
	for(i = 0 ;i < Xres*20; i++)
	{
		*((char *)addr+0)=(char)0xff;    // 0x000000ff, BLUE
		*((char *)addr+1)=(char)0x00;
		*((char *)addr+2)=(char)0x00;
		*((char *)addr+3)=(char)0x00;	
		addr +=1;	
	...
	for(i = 0 ;i < Xres*20; i++)
	{
		*((char *)addr+0)=(char)0xff;    // 0x00ffffff, WHITE
		*((char *)addr+1)=(char)0xff;
		*((char *)addr+2)=(char)0xff;
		*((char *)addr+3)=(char)0x00;	
		addr +=1;	
